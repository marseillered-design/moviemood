...
similarGrid.innerHTML = similar.results.slice(0, 12).map(m =>  `<div class="similar-card" onclick="window.open('movie.html?id=${m.id}&type=${mediaType}&region=${region}', '_blank')"> <img src="${m.poster_path ? TMDB_IMAGE_BASE_URL + m.poster_path : ''}" alt="${m.title || m.name}"> <span>${m.title || m.name}</span> </div>`).join('');
}
// Close modal
